...
def create_test_workflows():
    ...
    try:
        ...
        # Connect to Neo4j
        neo4j_client = neo4j(
            uri="bolt://localhost:7687", user="neo4j", password="testpassword", name="test_data_creator"
        )
        ...
        CREATE (p)-[:STARTS]->(a)
        """,
            {"prompt_id": prompt_1_id, "first_action_id": actions_1[0]["id"]},
        )
        ...
        for i in range(len(actions_1) - 1):
            ...
            CREATE (a1)-[:NEXT]->(a2)
            """,
                {"action1_id": actions_1[i]["id"], "action2_id": actions_1[i + 1]["id"]},
            )
        ...
        for i in [1, 2]:
            ...
            """,
                {"dependent_id": actions_1[i]["id"], "dependency_id": actions_1[0]["id"]},
            )
        ...
        CREATE (p)-[:STARTS]->(a)
        """,
            {"prompt_id": prompt_2_id, "first_action_id": actions_2[0]["id"]},
        )
        ...
        for i in range(len(actions_2) - 1):
            ...
            """,
                {"action1_id": actions_2[i]["id"], "action2_id": actions_2[i + 1]["id"]},
            )
        ...
        """,
            {"final_id": actions_2[3]["id"], "cleaned_id": actions_2[1]["id"]},
        )
