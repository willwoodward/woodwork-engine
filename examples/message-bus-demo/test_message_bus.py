...
class TestInputComponent(component):
    ...
    async def process_input(self, user_input: str):
        ...
        result = await self.emit(
            "input_received", {"input": user_input, "timestamp": time.time(), "component": self.name}
        )
...
class TestAgentComponent(component):
    ...
    async def process(self, input_data):
        ...
        await self.emit("response_generated", {"response": response, "timestamp": time.time(), "component": self.name})
...
async def test_message_routing():
    ...
    TestAgentComponent(config={"to": ["test_output"]})
...
async def test_streaming_integration():
    ...
    agent_comp = TestAgentComponent(config={"streaming": True, "streaming_output": True, "to": ["test_output"]})
    output_comp = TestOutputComponent(config={"streaming": True, "streaming_input": True})
...
async def test_performance():
    ...
    for i in range(message_count):
        envelope = create_hook_message(
            session_id="perf-test", event_type="performance_test", payload={"message_id": i}, sender_component="test"
        )
