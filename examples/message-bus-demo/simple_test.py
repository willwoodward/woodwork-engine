...
async def main():
    ...
    try:
        ...
        success = await router.route_component_output(
            source_component="input", event_type="input_received", data="Hello, message bus!", session_id="test-session"
        )
        ...
        test_comp = component(name="test", component="agents", type="test", config={"to": ["output"]})
        ...
        log.info(
            f"✅ Final message bus stats: messages_published={bus_stats['messages_published']}, healthy={message_bus.is_healthy()}"
        )
