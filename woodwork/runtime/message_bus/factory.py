...
class MessageBusFactory:
    ...
    def _create_in_memory_bus(config: Dict[str, Any]) -> MessageBusInterface:
        ...
        log.debug(
            "[MessageBusFactory] Creating InMemoryMessageBus: queue_size=%d, retries=%d", max_queue_size, max_retries
        )

        return InMemoryMessageBus(max_queue_size=max_queue_size, max_retries=max_retries)
...
def set_global_message_bus(message_bus: MessageBusInterface) -> None:
    ...
    log.info("[MessageBusFactory] Set custom global message bus: %s", type(message_bus).__name__)
...
def configure_for_development():
    ...
    configure_global_message_bus({"type": "memory", "max_queue_size": 1000, "max_retries": 2})
