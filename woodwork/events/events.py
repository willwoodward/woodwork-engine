...
class EventManager:
    ...
    def on_event(self, event: str, listener: Listener) -> None:
        ...
        log.debug(
            f"[EventManager] Registered EVENT listener for '{event}'. Total events for this event: {len(self._events[event])}"
        )
    ...
    def on_hook(self, event: str, listener: Listener) -> None:
        ...
        log.debug(
            f"[EventManager] Registered HOOK listener for '{event}'. Total hooks for this event: {len(self._hooks[event])}"
        )
    ...
    def on_pipe(self, event: str, listener: Listener) -> None:
        ...
        log.debug(
            f"[EventManager] Registered PIPE listener for '{event}'. Total pipes for this event: {len(self._pipes[event])}"
        )
    ...
    async def emit(self, event: str, data: Any = None) -> Any:
        ...
        hook_tasks = [self._execute_listener(hook, payload, event, "hook") for hook in self._hooks[event]]
