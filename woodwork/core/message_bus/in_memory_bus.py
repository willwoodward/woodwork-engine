...
from .interface import MessageBusInterface, MessageEnvelope, MessagePattern
...
class InMemoryMessageBus(MessageBusInterface):
    ...
    def __init__(self, max_queue_size: int = 10000, max_retries: int = 3):
        ...
        log.debug(
            "[InMemoryMessageBus] Initialized with max_queue_size=%d, max_retries=%d", max_queue_size, max_retries
        )
    ...
    async def stop(self) -> None:
        ...
        log.info("[InMemoryMessageBus] Stopped after %.2f seconds. Final stats: %s", uptime, self.get_stats())
    ...
    async def publish(self, envelope: MessageEnvelope) -> bool:
        ...
        log.debug(
            "[InMemoryMessageBus] Publishing '%s' to %d subscribers (session: %s)",
            topic,
            len(subscriber_ids),
            envelope.session_id,
        )
        ...
        log.debug(
            "[InMemoryMessageBus] Published '%s': %d delivered, %d failed in %.2fms",
            topic,
            delivered_count,
            failed_count,
            delivery_time_ms,
    ...
    async def subscribe(self, topic: str, callback: Callable[[MessageEnvelope], None]) -> str:
        ...
        subscription = Subscription(
            subscription_id=subscription_id, topic=topic, callback=callback, created_at=time.time()
        )
        ...
        log.debug(
            "[InMemoryMessageBus] Subscribed %s to topic '%s'. Total subscriptions: %d",
            subscription_id,
            topic,
    ...
    async def unsubscribe(self, subscription_id: str) -> bool:
        ...
        log.debug(
            "[InMemoryMessageBus] Unsubscribed %s from topic '%s'. Messages processed: %d",
            subscription_id,
            subscription.topic,
    ...
    async def send_to_component(self, envelope: MessageEnvelope) -> bool:
        ...
        if not self.running:
            log.warning(
                "[InMemoryMessageBus] Not running, dropping component message: %s -> %s",
                envelope.sender_component,
        ...
        log.debug(
            "[InMemoryMessageBus] Sending '%s' from %s to %s (session: %s)",
            envelope.event_type,
            envelope.sender_component,
            envelope.target_component,
            envelope.session_id,
        ...
        if handler:
            try:
                ...
                log.debug("[InMemoryMessageBus] Delivered to %s in %.2fms", envelope.target_component, delivery_time_ms)
                ...
                if envelope.can_retry():
                    ...
                    log.debug(
                        "[InMemoryMessageBus] Queued for retry %d/%d: %s",
            ...
            log.debug(
                "[InMemoryMessageBus] Queued message for %s (queue size: %d)", envelope.target_component, len(queue)
            )
    ...
    def register_component_handler(self, component_id: str, handler: Callable[[MessageEnvelope], None]) -> None:
        ...
        handler_obj = ComponentHandler(component_id=component_id, handler=handler, registered_at=time.time())
        ...
        if queued_messages:
            ...
            log.debug("[InMemoryMessageBus] Delivering %d queued messages to %s", len(queued_messages), component_id)
    ...
    def unregister_component_handler(self, component_id: str) -> bool:
        ...
        log.debug(
            "[InMemoryMessageBus] Unregistered component %s. Messages processed: %d",
            component_id,
    ...
    async def _deliver_queued_messages(self, component_id: str, messages: List[MessageEnvelope]) -> None:
        ...
        log.debug(
            "[InMemoryMessageBus] Delivered queued messages to %s: %d success, %d failed",
            component_id,
            delivered,
            failed,
        )
    ...
    async def _cleanup_processor(self) -> None:
        ...
        while self.running:
            try:
                ...
                self.retry_queue = [envelope for envelope in self.retry_queue if not envelope.is_expired()]
                ...
                for component_id, queue in self.component_queues.items():
                    ...
                    if expired_messages:
                        log.debug(
                            "[InMemoryMessageBus] Cleaned %d expired messages from %s queue",
    ...
    def _dead_letter(self, envelope: MessageEnvelope, reason: str) -> None:
        ...
        self.dead_letter_queue.append(
            {"envelope": envelope.to_dict(), "reason": reason, "dead_lettered_at": time.time()}
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            **self.stats,
            ...
            "delivery_success_rate": (self.stats["messages_delivered"] / max(self.stats["messages_published"], 1)),
            # Current state
    ...
    def is_healthy(self) -> bool:
        ...
        if self.stats["delivery_failures"] > 100:
            log.warning("[InMemoryMessageBus] High delivery failure count: %d", self.stats["delivery_failures"])
            return False

        # Check for excessive dead letters
        if len(self.dead_letter_queue) > 50:
            log.warning("[InMemoryMessageBus] High dead letter count: %d", len(self.dead_letter_queue))
            return False
